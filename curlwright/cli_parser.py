...
Examples:
  ...
  python curlwright.py -c "curl https://example.com" --headless --timeout 60 --retries 5
        ...
        browser_group.add_argument(
            ...
            type=str,
            default=None,
            help="Override the bypass state path (default: ~/.curlwright/bypass-state.json)",
        )
        browser_group.add_argument(
            "--artifact-dir",
            type=str,
            default=None,
            help="Directory for failure artifacts like HTML, screenshots, and diagnostics",
        )
        browser_group.add_argument(
            "--profile-dir",
            type=str,
            default=None,
            help="Override the persistent Chromium profile directory (default: ~/.curlwright/browser-profile)",
        )
        browser_group.add_argument(
            "--no-persist-cookies",
            action="store_true",
            default=False,
            help="Disable automatic cookie load/save between runs",
        )
        browser_group.add_argument(
            "--bypass-attempts",
            type=int,
            default=3,
            help="Challenge-resolution attempts per request before the retry loop continues (default: 3)",
        )
        ...
        cookie_group.add_argument(
            ...
            type=str,
            default=None,
            help="Load cookies from a JSON file into the jar before the request",
        )
        cookie_group.add_argument(
            "--export-cookies",
            type=str,
            default=None,
            help="Write the captured session cookies to a JSON file after the request",
        )
        cookie_group.add_argument(
            "--clear-cookies",
            action="store_true",
            default=False,
            help="Clear the persistent cookie jar before the request",
        )
        ...
        retry_group.add_argument(
            "--delay", type=int, default=5, help="Delay between retries in seconds (default: 5)"
        )
        ...
        output_group.add_argument(
            ...
            action="store_true",
            default=False,
            help="Emit a structured JSON payload with response, attempt summary, and runtime metadata",
        )
        output_group.add_argument(
            "--sarif-output",
            type=str,
            default=None,
            help="Write a SARIF 2.1.0 report for CI/security tooling to the given path",
        )
